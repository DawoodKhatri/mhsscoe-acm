...
const EventDetails = ({ title, bodyCode }) => {
  let contents = {
    img: ({ src, alt }) => (
      <div className="relative overflow-hidden rounded-lg">
        <img className="opacity-80 w-full m-0" src={src} />
        {alt && (
          <div
          ...
          >
            <p className="text-xl m-5 italic text-center text-white">{alt}</p>
          </div>
        )}
      </div>
    ),
  ...
  return (
    <article>
      <Glassmorphism className="mb-8 p-8 pt-2 md:m-5">
        <h2
          className={`text-4xl pt-3 sm:text-5xl md:text-6xl lg:text-7xl md:pt-0 font-bold italic text-center text-primary-dark`}
        >
          {title}
        </h2>
      </Glassmorphism>
      ...
      <Row justify="center" className="sm:px-8 md:px-16">
        <Col
          ...
          lg={{ span: 16, order: 0 }}
        >
          <Glassmorphism className="prose max-w-full w-full p-8 my-5">
